...
const ServiceVideoInspecaoPage = () => {
  useEffect(() => {
    document.title = "Vídeo Inspeção de Tubulações no Morumbi | Protec";
    let metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute("content", "Inspeção detalhada de tubulações com câmara de alta resolução. Diagnóstico exato de entupimentos sem necessidade de quebrar paredes ou pisos.");
    } else {
      metaDescription = document.createElement('meta');
      metaDescription.name = "description";
      metaDescription.content = "Inspeção detalhada de tubulações com câmara de alta resolução. Diagnóstico exato de entupimentos sem necessidade de quebrar paredes ou pisos.";
    ...
    <main className="bg-background min-h-screen">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="/image/hero.png" alt="Técnico operando equipamento de vídeo inspeção" className="w-full h-full object-cover" loading="eager" fetchPriority="high" />
          ...
          <div className="max-w-3xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold text-foreground leading-tight tracking-tight mb-6">
                Vídeo Inspeção de Tubulação no <span className="text-accent">Morumbi</span>
              </h1>
              <h2 className="text-xl sm:text-2xl text-muted-foreground font-medium mb-8 leading-relaxed">
                Diagnóstico 100% preciso com câmera de alta resolução. Veja o problema por dentro <strong className="text-foreground">sem destruir o piso.</strong>
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <ServiceAreasModal serviceName="Vídeo Inspeção" baseSlug="/video-inspecao-em" />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-20 lg:py-32 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-16 items-start">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="prose prose-lg dark:prose-invert prose-headings:font-display prose-headings:font-bold prose-a:text-accent max-w-none">
            <h2 className="text-3xl lg:text-4xl text-foreground mb-6">Pare de adivinhar onde está o <strong className="text-accent">problema</strong></h2>
            <p className="text-muted-foreground">O serviço de <strong>Vídeo Inspeção de Esgotos e Tubulações</strong> revolucionou o mercado. Com ele, mapeamos todo o encanamento e detetamos falhas estruturais, amassamentos, raízes intrusas e entupimentos crónicos com exatidão cirúrgica, poupando tempo e dinheiro com obras desnecessárias.</p>
            <h3 className="text-2xl text-foreground mt-8 mb-4">Ideal para Condomínios e Seguros</h3>
            <p className="text-muted-foreground">O <strong>laudo técnico</strong> gerado após a inspeção é aceito por seguradoras e administradoras de condomínios, servindo como prova para acionamento de garantias e coberturas.</p>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="lg:sticky lg:top-32">
            <div className="rounded-3xl overflow-hidden shadow-2xl border border-border bg-card">
              <img src="/image/hero.png" alt="Câmera de vídeo inspeção em ação" className="w-full h-80 object-cover" loading="lazy" />
              <div className="p-8">
                <h3 className="font-display font-bold text-2xl text-foreground mb-4">Laudo Técnico e Filmagem</h3>
                <div className="space-y-4">
                  <div className="flex items-center gap-4 text-muted-foreground"><Camera className="w-5 h-5 text-accent" /><p className="text-sm font-medium">Equipamento de última geração com imagem nítida.</p></div>
                  <div className="flex items-center gap-4 text-muted-foreground"><Video className="w-5 h-5 text-accent" /><p className="text-sm font-medium">Gravação digital em alta resolução.</p></div>
                  <div className="flex items-center gap-4 text-muted-foreground"><FileCheck className="w-5 h-5 text-accent" /><p className="text-sm font-medium">Documentação válida para condomínios e perícias.</p></div>
                </div>
                <a href="https://wa.me/5511940103334" target="_blank" rel="noopener noreferrer" className="block mt-8">
                  <Button className="w-full h-12 text-base font-bold bg-green-500 hover:bg-green-600 text-white">Solicitar Orçamento</Button>
                </a>
              </div>
            </div>
          </motion.div>
      ...
      <Suspense fallback={<div className="py-16 flex justify-center text-muted-foreground">A carregar áreas de atendimento...</div>}>
        <ServiceAreasList serviceName="Vídeo Inspeção" baseSlug="/video-inspecao-em" />
      </Suspense>

      <section className="py-20 bg-secondary/30 border-t border-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12"><h2 className="font-display font-extrabold text-3xl md:text-4xl text-foreground">Perguntas sobre <span className="text-accent">Vídeo Inspeção</span></h2></div>
